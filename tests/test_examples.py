...
def test_fixture_binding_usage_example_runs(pytester: pytest.Pytester) -> None:
    example_code = (ROOT / "examples" / "fixture_binding_usage.py").read_text(encoding="utf-8")
    snapshot_text = (ROOT / "examples" / "warmup.snapshot.json").read_text(encoding="utf-8")

    pytester.makepyfile(test_fixture_binding_usage_example=example_code)
    pytester.path.joinpath("warmup.snapshot.json").write_text(snapshot_text, encoding="utf-8")

    result = pytester.runpytest(
        "--warmup-snapshot-for=fixture-binding-usage=warmup.snapshot.json",
        "-q",
    )
    result.assert_outcomes(passed=1)
